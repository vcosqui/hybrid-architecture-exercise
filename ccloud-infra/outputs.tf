output "resource-ids" {
  ...
  $ confluent kafka topic produce ${confluent_kafka_topic.orders.topic_name} --value-format avro --parse-key --delimiter "#" --environment ${confluent_environment.dev.id} --cluster ${confluent_kafka_cluster.standard.id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}"
  ...
  $ confluent kafka topic consume ${confluent_kafka_topic.orders.topic_name} --value-format avro --print-key --from-beginning --environment ${confluent_environment.dev.id} --cluster ${confluent_kafka_cluster.standard.id} --api-key "${confluent_api_key.app-consumer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-consumer-kafka-api-key.secret}"
  # When you are done, press 'Ctrl-C'.


  # confluent kafka topic produce user_added2 --parse-key --delimiter "#" --environment env-j5d728 --cluster lkc-v7g02p --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}" --value-format avro --schema-id=100001 --sr-api-key="$TF_VAR_confluent_schema_registry_api_key" --sr-api-secret="$TF_VAR_confluent_schema_registry_api_secret"
  EOT
...
#}
# $ confluent kafka topic produce ${confluent_kafka_topic.orders.topic_name} --value-format avro --parse-key --delimiter "#" --environment ${confluent_environment.dev.id} --cluster ${confluent_kafka_cluster.standard.id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}"
# 1#{"my_field1":1,"my_field2":1.1,"my_field3":"18500"}